...
next_index = len([d for d in Path("log").iterdir() 
                if d.name.startswith(f"{room_file_name}_")]) 
...
foam_new_path = Path(f"log/{room_file_name}_{next_index}/base/{room_file_name}_{next_index}.foam")
