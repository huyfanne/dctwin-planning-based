...
class ConfigBuilder:
    ...
    def make_logging_config(self, log_dir: Path, level: int, verbose: bool) -> None:
    ...
    def make_plant_loop_observations(
        ...
        for (
            chilled_water_loop_name,
            chilled_water_loop,
        ) in self.building.constructions.plant.chilled_water_loops.items():
    ...
    def make_secondary_chilled_water_loop_observations(
        ...
        for (
            secondary_chilled_water_loop_name,
            secondary_chilled_water_loop,
        ) in self.device_key_map["secondary chilled water loops"].items():
            # observe chilled water loop supply temperature
            self._make_observation(
                exposed=exposed,
                variable_name=f"{secondary_chilled_water_loop_name} supply temperature".lower(),
                key_value=secondary_chilled_water_loop["supply temperature"].split(":")[
                    0
                ],
                output_variable_name="System Node Temperature",
                reporting_frequency="timestep",
            ...
            self._make_observation(
                ...
                key_value=secondary_chilled_water_loop["return temperature"].split(":")[
                    0
                ],
                output_variable_name="System Node Temperature",
                reporting_frequency="timestep",
            ...
            self._make_observation(
                ...
                key_value=secondary_chilled_water_loop["supply flow rate"].split(":")[
                    0
                ],
                output_variable_name="System Node Mass Flow Rate",
                reporting_frequency="timestep",
            ...
            self._make_observation(
                ...
                key_value=secondary_chilled_water_loop["return flow rate"].split(":")[
                    0
                ],
                output_variable_name="System Node Mass Flow Rate",
                reporting_frequency="timestep",
    ...
    def make_dehumidifier_observations(
        ...
        for dehumidifier_name, dehumidifier in self.device_key_map[
            "dehumidifiers"
        ].items():
            # observe inlet air temperature
            self._make_observation(
    ...
    def make_pump_observations(
        ...
        for pump_name, pump in self.device_key_map["pumps"].items():
        ...
        for chiller_name, chiller in self.device_key_map["chillers"].items():
            ...
            if self.device_key_map["condenser water loops"]:
                self._make_observation(
                    ...
                    key_value=chiller["condenser water return temperature"].split(":")[
                        0
                    ],
    ...
    def make_thermal_storage_observations(
        ...
        for storage_name, storage in self.device_key_map[
        ...
        for chilled_water_loop_name, chilled_water_loop in chilled_water_loops.items():
            for branch_name, branch in chilled_water_loop.supply_branches.items():
                self._make_actions(
                    ...
                    lb=lb,
                    ub=ub,
                    input_source=schedule_dir.joinpath(f"{branch_name}.json"),
    ...
    def make_acu_on_off_schedules(
        ...
        for acu_name, acu in self.device_key_map["acus"].items():
            ...
            ).replace("\\", "/")  # convert to unix style path
        ...
        for zone_name, zone in self.building.constructions.zones.items():
            if zone.constructions.cdus is not None:
                for cdu_name, cdu in zone.constructions.cdus.items():
                    self._make_actions(
                        variable_name=f"{cdu.uid} cooling water supply temperature".lower(),
                        control_type=device_values.get(f"{cdu.uid}", {}).get(
                            "control_type", control_type
                        ),
                        method=device_values.get(f"{cdu.uid}", {}).get(
                            "normalize_method", normalize_method
                        ),
                        lb=device_values.get(f"{cdu.uid}", {}).get("lb", lb),
                        ub=device_values.get(f"{cdu.uid}", {}).get("ub", ub),
                        default_unnormed_value=device_values.get(f"{cdu.uid}", {}).get(
                            "default_unnormed_value", default_unnormed_value
                        ),
                    ) if device_values.get(f"{cdu.uid}", {}).get(
                        "disable", disable
                    ) == False else None
