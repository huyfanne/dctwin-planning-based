...
class CoSimManager:
    ...
    def _post_processing(
        ...
        for crac_id, _ in self.cfd_manager.object_mesh_index["cracs"].items():
            ...
            cfd_log_dict.update({f"{crac_id} (m3/s)": round(crac_flow_rates[crac_id], 3)})

        for server_id, _ in self.cfd_manager.object_mesh_index["servers"].items():
            total_server_powers += server_powers[server_id]
            total_server_flow_rates += server_flow_rates[server_id]
    ...
    def _map_boundary_conditions(self, parsed_actions: Dict) -> Dict:
        ...
        for crac in self.eplus_manager.idf_parser.epm.AirLoopHVAC:
            ...
            boundary_conditions["crac_flow_rates"][uid] = parsed_actions[f"{uid}_flow_rate"] / self.rho_air
