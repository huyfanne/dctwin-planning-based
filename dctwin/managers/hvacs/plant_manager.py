...
class PlantManager(nn.Module):
    ...
    @staticmethod
    def _set_branch_inlet_properties(data: Batch, current_branch, last_branch, loop_id: str) -> None:
        last_branch_id = list(last_branch.keys())[0]
        current_branch_id = list(current_branch.keys())[0]
        if len(data.acts[loop_id].supply_temperature_sp) != 0:
            ...
            data.obs_next.plants[last_branch_id].outlet_temperature = data.obs.plants[last_branch_id].inlet_temperature
        data.obs_next.plants[current_branch_id].inlet_temperature = data.obs_next.plants[last_branch_id].outlet_temperature

    @staticmethod
    def _update_spliter(data: Batch, split_branch_id: str, inlet_branch: Dict) -> None:
        inlet_branch_id = list(inlet_branch.keys())[0]
        data.obs_next.plants[split_branch_id].inlet_temperature = data.obs_next.plants[inlet_branch_id].outlet_temperature
            ...
            for component_id, component in branch.components.pumps.items():
                if data.acts[component_id].on_off_schedule == 1:
                    component.model.forward(
                        requested_mass_flow_rate=data.acts[component_id].mass_flow_rate,
                        external_inputs=data.inps
                    )
            ...
            for component_id, component in branch.components.chillers.items():
                if data.acts[component_id].on_off_schedule == 1:
                    ...
                    chiller_power = component.model.forward(
                        ...
                        chw_sp=data.acts[component_id].supply_temperature_sp,
                        cw_sp=cw_sp,
                    )
        ...
        for branch_id, branch in this_branches.items():
            if branch.side == "inlet":
                ...
                self._set_branch_inlet_properties(
                    data=data, current_branch=inlet_branch, last_branch=last_branch, loop_id=loop_id,
                )
