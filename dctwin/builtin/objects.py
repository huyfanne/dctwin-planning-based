...
from dctwin.models.objects import ACUFace, ACUModel, RackModel
from dctwin.models.server import ServerModel

acu_model_1 = ACUModel(
    size=Size(dx=1.6, dy=0.8, dz=2),
    supply_face=ACUFace(
        side='front', width=1.4, length=1.8, offset=Vertex(x=0, y=0, z=0)
    ),
    return_face=ACUFace(
        side='top', width=1.4, length=0.8, offset=Vertex(x=0, y=0, z=0)
    ),
)
