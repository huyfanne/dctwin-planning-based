...
class EplusCloudEnv(EPlusEnv):
    ...
    def _set_cloud_environ(self) -> None:
        ...
        """
        cloud_env.cloud.job_duration_estimator_config = self.cloud_configs.job_duration_estimator_config
    ...
    def _get_actions_to_sent(self) -> Union[List[float], Dict]:
        ...
        capacity_budget = self.inspect_action_by_name(
            action_name="capacity budget imdc level 2 cluster",
            use_unnormed=True
        )
        eplus_actions = [
            self.inspect_action_by_name(action_name=act.variable_name, use_unnormed=True) for act in self.actions
            if act.variable_name != "capacity budget imdc level 2 cluster"
        ]
        actions = {
            "capacity budget": capacity_budget,
            "eplus actions": eplus_actions
        }
        return actions

    def _run_simulation(
        self,
        parsed_actions: np.ndarray | List[float] | Dict
    ...
    def format_obs(self) -> dict:
        ...
        for acu_name, acu in self.device_key_mapping["acus"].items():
            ...
            data[acu["cooling coil"]["inlet air temperature"]] = self.inspect_current_observation(
                observation_name=f"{acu_name.lower()} cooling coil inlet air temperature",
                use_unnormed=True
            )
            data[acu["cooling coil"]["air mass flow rate"]] = self.inspect_current_observation(
                observation_name=f"{acu_name.lower()} cooling coil air mass flow rate",
                use_unnormed=True
            )
            data[acu["cooling coil"]["outlet air temperature"]] = self.inspect_current_observation(
                observation_name=f"{acu_name.lower()} cooling coil outlet air temperature",
                use_unnormed=True
            )
            data[acu["cooling coil"]["inlet water temperature"]] = self.inspect_current_observation(
                observation_name=f"{acu_name.lower()} cooling coil inlet water temperature",
                use_unnormed=True
            )
            data[acu["cooling coil"]["water mass flow rate"]] = self.inspect_current_observation(
        ...
        for chiller_name, chiller in self.device_key_mapping["chillers"].items():
            ...
            data[chiller["chilled water supply temperature"]] = self.inspect_current_observation(
                observation_name=f"{chiller_name.lower()} chilled water supply temperature",
                use_unnormed=True
            )
            data[chiller["condenser water supply temperature"]] = self.inspect_current_observation(
        ...
        for tower_name, tower in self.device_key_mapping["cooling towers"].items():
            ...
            data[tower["cooling tower air flow rate ratio"]] = self.inspect_current_observation(
                observation_name=f"{tower_name.lower()} air flow rate ratio",
                use_unnormed=True
            )
            data[tower["outside air wetbulb temperature"]] = self.inspect_current_observation(
        ...
        for center_name, center in self.device_key_mapping["electrical load centers"].items():
        ...
        data["outdoor air drybulb temperature"] = self.inspect_current_observation(
            observation_name="site outdoor air drybulb temperature",
            use_unnormed=True
        )
        ...
        data["wind speed"] = self.inspect_current_observation(
            observation_name="site wind speed",
            use_unnormed=True
        )
        data["diffuse solar radiation rate per area"] = self.inspect_current_observation(
        ...
        data["ground reflected solar radiation rate per area"] = self.inspect_current_observation(
