...
class ElectricSystemBuilder:
    ...
    def _make_generator(self, generator_config: PhotovoltaicGenerator):
        ...
        generator_obj = self.model.newidfobject(
            key="Generator:Photovoltaic".upper(),
            defaultvalues=True
        )
        generator_obj["Name"] = generator_config.uid
        generator_obj["Number_of_Modules_in_Series"] = generator_config.number_of_modules_in_series
        generator_obj["Number_of_Series_Strings_in_Parallel"] = generator_config.number_of_series_strings_in_parallel
        generator_obj["Photovoltaic_Performance_Object_Type"] = "PhotovoltaicPerformance:EquivalentOne-Diode"
        generator_obj["Module_Performance_Name"] = performance_model_obj.Name
        generator_obj["Surface_Name"] = generator_config.surface_name
        generator_obj["Heat_Transfer_Integration_Mode"] = generator_config.heat_transfer_integration_mode
        ...
        surface_obj = self.model.getobject(
            ...
            name=generator_config.surface_name
        )
        surface_obj["Outside_Boundary_Condition"] = "OtherSideConditionsModel"
        surface_obj["Outside_Boundary_Condition_Object"] = (f"{generator_config.surface_name} "
        ...
        self.model.newidfobject(
            ...
            Boundary_Conditions_Model_Name=(f"{generator_config.surface_name} outside boundary condition object"),
    ...
    def _make_generators(self, electric_load_center_name: str, generators: Dict[str, PhotovoltaicGenerator]):
        ...
        for idx, (generator_name, generator) in enumerate(generators.items()):
            ...
            obj[f"Generator_{int(idx) + 1}_Rated_Electric_Power_Output"] = generator.rated_electric_power_output
    ...
    def _make_electric_load_center(
        ...
        obj = self.model.newidfobject(
            key="ElectricLoadCenter:Distribution".upper(),
            defaultvalues=True
        )
        self._make_generators(electric_load_center_name, config.generators)
        self._make_inverter(config.inverter)
        self._make_battery(config.electrical_storage) if config.electrical_storage else None
        ...
        obj["Electrical_Storage_Object_Name"] = config.electrical_storage.name if config.electrical_storage else ""
        obj["Transformer_Object_Name"] = config.transformer.name if config.transformer else ""
