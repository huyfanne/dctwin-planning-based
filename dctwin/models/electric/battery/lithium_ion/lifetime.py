...
class NMCLifetimeModel(nn.Module):
    ...
    def __init__(self, dt_hr: torch.Tensor | float):
        ...
        self.day_age_of_battery = torch.tensor(
            0.0, dtype=torch.float32
        )  # Ensuring it's a float tensor # days, age of the battery
        ...
        self.dq_relative_li_old = torch.tensor(
            100.0
        )  # %, SEI degradation at previous timestep
        self.dq_relative_neg_old = torch.tensor(
            100.0
        )  # %, cycle degradation at previous timestep
        self.dod_max = torch.tensor(50.0)  # %, maximum depth of discharge
        self.n_cycles_prev_day = torch.tensor(
            0.0
        )  # number of cycles in the previous day
    ...
    def bilinear(self, dod, cycle_number):
        ...
        dod = (
            torch.tensor(dod, dtype=torch.float32)
            if not isinstance(dod, torch.Tensor)
            else dod
        )
        cycle_number = (
            torch.tensor(cycle_number, dtype=torch.float32)
            if not isinstance(cycle_number, torch.Tensor)
            else cycle_number
        )
    ...
    def runQli(self, T_battery: torch.Tensor | float):
        ...
        d0_t = self.d0_ref * torch.exp(
            -(self.Ea_d0_1 / self.Rug) * (1 / T_battery - 1 / self.T_ref)
            - (self.Ea_d0_2 / self.Rug)
            * torch.pow(torch.tensor(1 / T_battery - 1 / self.T_ref), 2)
        )

        if self.day_age_of_battery > 0:
            k_cal = (0.5 * b1) / torch.sqrt(self.day_age_of_battery) + (
                b3 / self.tau_b3
            ) * torch.exp(-(self.day_age_of_battery / self.tau_b3))
    ...
    def runQneg(self):
        ...
        if self.n_cycles > 0:
            dq_new = (
                c2 / torch.sqrt(c0 * c0 - 2 * c2 * c0 * self.n_cycles) * dn_cycles
                + self.dq_relative_neg_old
            )
    ...
    def integrateDegLoss(
        self, dod: torch.Tensor | float, T_battery: torch.Tensor | float
    ):
    ...
    def integrateDegParams(
        ...
        b1_dt_el = (
            self.b1_ref
            * torch.exp(-(self.Ea_b1 / self.Rug) * (1.0 / T_battery - 1.0 / self.T_ref))
            * torch.exp(
                (self.alpha_a_b1 * self.F / self.Rug)
                * (U_neg / T_battery - self.Uneg_ref / self.T_ref)
            )
            * torch.exp(self.gamma * torch.pow(dod_max, self.beta_b1))
            * dt_day
        )
        b2_dt_el = (
            self.b2_ref
            * torch.exp(
                -(self.Ea_b_2 / self.Rug) * (1.0 / T_battery - 1.0 / self.T_ref)
            )
            * dt_day
        )
        b3_dt_el = (
            self.b3_ref
            * torch.exp(-(self.Ea_b3 / self.Rug) * (1.0 / T_battery - 1.0 / self.T_ref))
            * torch.exp(
                (self.alpha_a_b3 * self.F / self.Rug)
                * (V_oc / T_battery - self.V_ref / self.T_ref)
            )
            * (1 + self.theta * dod_max)
            * dt_day
        )
        ...
        c2_dt_el = (
            self.c2_ref
            * torch.exp(-(self.Ea_c2 / self.Rug) * (1.0 / T_battery - 1.0 / self.T_ref))
            * torch.pow(0.01 * self.dod_max, self.beta_c2)
            * dt_day
        )
        c0_dt_el = (
            self.c0_ref
            * torch.exp(-self.Ea_c0_ref / self.Rug * (1 / T_battery - 1 / self.T_ref))
            * dt_day
        )
    ...
    def rainflow(self, dod: torch.Tensor | float):
        ...
        while len(self.rainflow_peaks) >= 3:
            ...
            if Ylt <= Xlt:
                ...
                self.n_cycles += 1
                self.average_range = (
                    self.average_range * (self.n_cycles - 1) + Ylt
                ) / self.n_cycles
                dq = self.bilinear(self.average_range, self.n_cycles) - self.bilinear(
                    self.average_range, self.n_cycles + 1
                )
                if dq > 0:
    ...
    def run(
        ...
        """
        logger.info(
            f"Run started with q_relative: {self.q_relative}, DOD change: {charge_changed}"
        )
        if prev_dod is None:
        ...
        prev_dod = (
            torch.tensor(prev_dod, dtype=torch.float32)
            if not isinstance(prev_dod, torch.Tensor)
            else prev_dod
        )
        ...
        if new_cum_dt > 1 + 1e-7:
            ...
            DOD_at_end_of_day = (
                dod - prev_dod
            ) / dt_day * dt_day_to_end_of_day + prev_dod
