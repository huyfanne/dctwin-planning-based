...
class HeatLoadManager(nn.Module):
    ...
    def _init_models(self):
        ...
        for zone_name, zone in self.zones.items():
            ...
            for ite_name, ite in zone.constructions.heat_gains.ites.items():
                ...
                zone_models["ites"][ite.uid] = ITEModel(ite)
                self.add_module(ite.uid, zone_models["ites"][ite.uid])
        return zone_models

    def _sim_zone_ite_heat_gains(
        self, zone_cpu_load_schedules: Batch
    ):
        zone_ite_heat_load = 0.0
        for ite_name, cpu_load_schedule in zone_cpu_load_schedules.items():
            zone_ite_heat_load += self.models["ites"][ite_name](
                cpu_load_schedule,
                None
            )
        return zone_ite_heat_load
    ...
    def forward(
        ...
        for zone_name, zone_cpu_schedule in cpu_load_schedule.items():
            ite_heat_loads[zone_name] = self._sim_zone_ite_heat_gains(
                zone_cpu_load_schedules=zone_cpu_schedule
            )
        return Batch(
            ite_heat_loads=ite_heat_loads
        )
