...
class PipeModel(nn.Module):
    ...
    def __init__(self, pipe: Pipe, sub_pipe_diameter: float = None) -> None:
        super().__init__()
        ...
        self.sub_pipe_diameter = (
            torch.tensor(sub_pipe_diameter) if sub_pipe_diameter else None
        )  # for tee
        ...
        self.relative_roughness = torch.tensor(
            pipe.geometry.roughness / pipe.geometry.pipe_diameter
        )
    ...
    def sim(
        ...
        """
        fluid_velocity = (
            main_pipe_mass_flow_rate
            / self.fluid_density
            / (torch.pi * self.pipe_diameter**2 / 4)
        )
        if self.channel_type == "straight":
            friction_factor = self.friction_factor(fluid_velocity=fluid_velocity)
            friction_coefficient = friction_factor * (
