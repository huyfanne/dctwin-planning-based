...
class DehumidifierModel(nn.Module):
    ...
    def forward(
        ...
        if relative_humidity_setpoint > inlet_relative_humidity:
            ...
            return torch.zeros(1,), inlet_dry_bulb_temperature, inlet_relative_humidity, torch.zeros(1,), torch.zeros(1,)
        ...
        hfg = PropsSI(
            'H',
            'T', 273.15 + inlet_dry_bulb_temperature.item(),
            'Q', 1,
            'water'
        ) - PropsSI(
            'H',
        ...
        water_removal_met = rho_air * self.config.cooling.rated_air_flow_rate * (
        ...
        water_removal_mass_rate = self.config.cooling.rated_water_removal * water_removal_rate_factor
        ...
        average_water_removal_rate = steady_state_water_removal_rate * part_load_ratio  # kg/s
        ...
        outlet_relative_humidity = torch.tensor(outlet_relative_humidity, device=self.device)
        mass_air_flow_rate = torch.tensor(rho_air * self.config.cooling.rated_air_flow_rate)

        return electric_power_avg, outlet_temperature, outlet_relative_humidity, average_water_removal_rate, mass_air_flow_rate
