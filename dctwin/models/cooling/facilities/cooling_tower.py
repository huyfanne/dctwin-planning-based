...
class VariableSpeedCoolingTowerModel(nn.Module):
    ...
    def _calibrate_flow(self) -> None:
        ...
        max_water_flow_rate_ratio = torch.tensor(
            0.5, dtype=torch.float32
        )  # maximum water flow rate ratio which yields desired approach temp
        t_approach = torch.tensor(
            0.0, dtype=torch.float32
        )  # temporary tower approach temp variable [C]
        ...
        model_water_flow_ratio_max = (
            self.config.cooling.cell_maximum_water_flow_rate_fraction * 4
        )  # maximum water flow rate ratio used for model calibration
        ...
        while (
            ...
            and max_water_flow_rate_ratio <= model_water_flow_ratio_max
        ):
            ...
            t_approach = self._calc_approach_temp(
                ...
                torch.tensor(
                    self.config.cooling.design_inlet_air_wet_bulb_temperature,
                    dtype=torch.float32,
                ),
                torch.tensor(
                    self.config.cooling.design_range_temperature, dtype=torch.float32
                ),
            )
            ...
            if (
                ...
                and t_approach < self.config.cooling.design_approach_temperature
            ) or (max_water_flow_rate_ratio >= model_water_flow_ratio_max):
                model_calibrated = False
                break

        water_flow_ratio = torch.tensor(
            0.0, dtype=torch.float32
        )  # Tower water flow rate ratio found during model calibration
        ...
        if model_calibrated:
            ...
            flag, water_flow_ratio = solve_root(
                self.tol,
                self.max_root_finding_iter,
                1,
                water_flow_ratio,
                f_calib,
                0.5,
                max_water_flow_rate_ratio,
            )
        ...
        self.calibrated_water_flow_rate = (
            self.config.cooling.design_water_flow_rate / water_flow_ratio
        )
        ...
        self.tower_nominal_capacity = (
            (rho * self.config.cooling.design_water_flow_rate)
            * cp
            * self.config.cooling.design_range_temperature
        )

        self.free_conv_air_flow_rate = (
            self.config.cooling.minimum_air_flow_rate_ratio
            * self.config.cooling.design_air_flow_rate
        )
    ...
    def _calc_approach_temp(
        ...
        t_approach = (
            self.yor_calc_coe[0]
            + self.yor_calc_coe[1] * outside_air_wet_bulb_temp
            + self.yor_calc_coe[2]
            * outside_air_wet_bulb_temp
            * outside_air_wet_bulb_temp
            + self.yor_calc_coe[3] * t_range
            + self.yor_calc_coe[4] * outside_air_wet_bulb_temp * t_range
            + self.yor_calc_coe[5]
            * outside_air_wet_bulb_temp
            * outside_air_wet_bulb_temp
            * t_range
            + self.yor_calc_coe[6] * t_range * t_range
            + self.yor_calc_coe[7] * outside_air_wet_bulb_temp * t_range * t_range
            + self.yor_calc_coe[8]
            * outside_air_wet_bulb_temp
            * outside_air_wet_bulb_temp
            * t_range
            * t_range
            + self.yor_calc_coe[9] * flow_factor
            + self.yor_calc_coe[10] * outside_air_wet_bulb_temp * flow_factor
            + self.yor_calc_coe[11]
            * outside_air_wet_bulb_temp
            * outside_air_wet_bulb_temp
            * flow_factor
            + self.yor_calc_coe[12] * t_range * flow_factor
            + self.yor_calc_coe[13] * outside_air_wet_bulb_temp * t_range * flow_factor
            + self.yor_calc_coe[14]
            * outside_air_wet_bulb_temp
            * outside_air_wet_bulb_temp
            * t_range
            * flow_factor
            + self.yor_calc_coe[15] * t_range * t_range * flow_factor
            + self.yor_calc_coe[16]
            * outside_air_wet_bulb_temp
            * t_range
            * t_range
            * flow_factor
            + self.yor_calc_coe[17]
            * outside_air_wet_bulb_temp
            * outside_air_wet_bulb_temp
            * t_range
            * t_range
            * flow_factor
            ...
            + self.yor_calc_coe[19]
            * outside_air_wet_bulb_temp
            * flow_factor
            * flow_factor
            + self.yor_calc_coe[20]
            * outside_air_wet_bulb_temp
            * outside_air_wet_bulb_temp
            * flow_factor
            * flow_factor
            + self.yor_calc_coe[21] * t_range * flow_factor * flow_factor
            + self.yor_calc_coe[22]
            * outside_air_wet_bulb_temp
            * t_range
            * flow_factor
            * flow_factor
            + self.yor_calc_coe[23]
            * outside_air_wet_bulb_temp
            * outside_air_wet_bulb_temp
            * t_range
            * flow_factor
            * flow_factor
            ...
            + self.yor_calc_coe[25]
            * outside_air_wet_bulb_temp
            * t_range
            * t_range
            * flow_factor
            * flow_factor
            + self.yor_calc_coe[26]
            * outside_air_wet_bulb_temp
            * outside_air_wet_bulb_temp
            * t_range
            * t_range
    ...
    def collect(self, data: dict) -> None:
        self.buffer.add(
            Batch(
                cooling_tower_return_water_temperature=data[
                    self.key_mapping["return water temperature"]
                ],
                cooling_tower_water_mass_flow_rate=data[
                    self.key_mapping["water mass flow rate"]
                ],
                cooling_tower_supply_water_temperature=data[
                    self.key_mapping["supply water temperature"]
                ],
                outside_air_wetbulb_temperature=data[
                    self.key_mapping["outside air wetbulb temperature"]
                ],
                cooling_tower_air_flow_rate_ratio=data[
                    self.key_mapping["air flow rate ratio"]
                ],
    ...
    def get_fluid_property(
        ...
        try:
            ...
            property_map = {"density": "D", "specific_heat": "C"}
            ...
            # Get the property
            prop = PropsSI(
                property_map[property_type], "T", temperature_k, "P", 101325, fluid_name
            )
            return prop
    ...
    def forward(
        ...
        def f(t_range):
            ...
            return (
                outside_air_wet_bulb_temp + t_approach + t_range
        ...
        # if flag = -2, Tr is returned as minimum value (0.001) and outlet temp = inlet temp - 0.001
            ...
            if self.single_setpoint:
                ...
                raise Exception(
                    "Unknown LoopDemandCalcScheme"
                )  # Raise an error if scheme is unknown
        ...
        if self.fan_power_f_air_flow_curve == 0:
            fan_power = (
                m_air_flow_rate_ratio**3
                * self.config.power.design_fan_power
                * num_cell_on
            ...
            fan_power = (
                ...
                * num_cell_on
                / self.config.cooling.number_of_cells
            )

        return (
            outlet_water_temp,
            fan_power,
        )  # Return the calculated outlet water temperature
    ...
    def solve(
        ...
        rho_water = torch.tensor(
            self.get_fluid_property(
                fluid_name="water", temperature=init_conv_temp, property_type="density"
            ),
            dtype=torch.float32,
        )

        des_water_mass_flow_rate = (
            self.config.cooling.design_water_flow_rate * rho_water
        )
        ...
        if des_water_mass_flow_rate > 0.0:
            ...
            num_cell_min = torch.minimum(
                (
                    water_mass_flow_rate / water_mass_flow_rate_per_cell_max + 0.999
                ).int(),
                torch.tensor(self.config.cooling.number_of_cells, dtype=torch.int32),
            )
            num_cell_max = torch.minimum(
                (
                    water_mass_flow_rate / water_mass_flow_rate_per_cell_min + 0.999
                ).int(),
                torch.tensor(self.config.cooling.number_of_cells, dtype=torch.int32),
            )
        else:
        ...
        water_flow_rate_ratio_capped = torch.tensor(
            0.0, dtype=torch.float32
        )  # Water flow rate ratio passed to VS tower model
        ...
        while incr_num_cell_flag:
            ...
            if outlet_water_temp_on > cw_supply_temp_setpoint:
                ...
                if (
                    ...
                    and water_mass_flow_rate / (num_cell_on + 1)
                    > water_mass_flow_rate_per_cell_min
                ):
                    num_cell_on += 1
                    water_mass_flow_rate_per_cell = water_mass_flow_rate / num_cell_on
        ...
        if outlet_water_temp_on < cw_supply_temp_setpoint:
            ...
            if outlet_water_temp_off > cw_supply_temp_setpoint:
                ...
                air_flow_rate_ratio = torch.tensor(
                    self.config.cooling.minimum_air_flow_rate_ratio, dtype=torch.float32
                )
                ...
                if outlet_water_temp_min < cw_supply_temp_setpoint:
                    ...
                    flag, air_flow_rate_ratio = solve_root(
                        ...
                        air_flow_rate_ratio,
                        f,
                        self.minimum_air_flow_rate_ratio,
                        torch.tensor(1.0),
                    )
                    ...
                    if self.fan_power_f_air_flow_curve == 0:
                        fan_power = (
                            air_flow_rate_ratio**3
                            * self.config.power.design_fan_power
                            * num_cell_on
                            / self.number_of_cells
                        )
                    else:
                        fan_curve_value = self.fan_power_of_air_flow_curve(
                            air_flow_rate_ratio
                        )
                        fan_power = (
                            torch.maximum(
        ...
        if error > torch.tensor(0.2, dtype=torch.float32):
            logger.warning(
                f"The set point temperature cannot be reached. Error: {error:.2f}"
            )
