...
class PumpModel(nn.Module):
    ...
    def forward(
        ...
        flow_fraction = torch.clip(flow_fraction, 0, 1)
        ...
        if self.learnable:
            ...
            if len(batch) > 3:
                flow_fraction = np.clip(
                    batch.pump_mass_flow_rate / (self.design_flow_rate * rho_water),
                    0,
                    1,
                )
                ...
                logger.warning(
                    f"Insufficient data for learning the pump model of {self.uid}."
                )
