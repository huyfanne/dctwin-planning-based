...
class FlowNetwork(nn.Module):
    ...
    def _init_models(self) -> Dict[str, Any]:
        ...
        for (
            supply_branch_name,
            supply_branch,
        ) in self.liquid_flow_loop.supply_branches.items():
        ...
        for (
            demand_branch_name,
            demand_branch,
        ) in self.liquid_flow_loop.demand_branches.items():
    ...
    def forward(self, data: Batch, zone_name: str) -> None:
        # compute the total mass flow rate supplied by the CDUs
        cdu_total_mass_flow_rate = torch.zeros(
            1,
        )
        cdu_supply_temperature = torch.zeros(
            1,
        )
        for (
            supply_branch_name,
            ...
            if supply_branch.components.cdus is not None:
                for cdu_name, cdu in supply_branch.components.cdus.items():
                    cdu_total_mass_flow_rate += data.acts[
                        cdu_name
                    ...
                    data.obs_next.zones[
                        cdu_name
                    ].cooling_water_supply_temperature = data.acts[
                        cdu_name
                    ].supply_temperature_sp
                    data.obs_next.zones[
                        cdu_name
                    ].cooling_water_mass_flow_rate = data.acts[
                        cdu_name
                    ].supply_mass_flow_rate_sp
        ...
        total_sensible_heat_load = torch.zeros(
            1,
        )
        cdu_return_temperature = torch.zeros(
            1,
        )
        for (
            demand_branch_name,
            ...
            if demand_branch.components.servers is not None:
                for server_name, server in demand_branch.components.servers.items():
                    data.obs_next.zones[
                        server_name
                    ].liquid_inlet_temperature = cdu_supply_temperature
                    ...
                    (
                        ...
                        inlet_liquid_mass_flow_rate=cdu_total_mass_flow_rate
                        / self.num_servers,
                    ...
                    data.obs_next.zones[
                        server_name
                    ].liquid_outlet_temperature = liquid_outlet_temperature
                    data.obs_next.zones[
                        server_name
                    ].max_chip_temperature = max_chip_temperature
